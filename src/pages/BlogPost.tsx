...
export function BlogPost() {
  ...
  if (!post) {
    return (
      <div className="min-h-screen bg-[#0a0a1a] text-white flex flex-col items-center justify-center p-6">
        ...
        <Link to="/blog" className="text-[#a020f0] hover:underline flex items-center gap-2">
  ...
  const renderContent = (content: string) => {
    ...
    return content.split('\n').map((line, i) => {
      ...
      let processedLine = line
        ...
        .replace(/\[(.*?)\]\((.*?)\)/g, '<a href="$2" class="text-[#a020f0] hover:underline">$1</a>');
  ...
  return (
    <div className="bg-[#0a0a1a] text-white min-h-screen pt-32 pb-24 selection:bg-[#a020f0]/30">
      <div className="max-w-4xl mx-auto px-6">
        {/* Breadcrumb */}
        <nav className="flex items-center gap-2 text-xs text-[#a0a0b8] uppercase tracking-widest mb-8 font-ui">
          <Link to="/blog" className="hover:text-white transition-colors">Blog</Link>
          <span>/</span>
          <span className="text-[#a020f0]">{post.category}</span>
          <span>/</span>
        ...
        <header className="mb-12">
          ...
          <div className="flex flex-wrap items-center gap-6 text-sm text-[#a0a0b8] font-ui">
            <div className="flex items-center gap-2">
              <div className="w-8 h-8 rounded-full bg-[#12122a] border border-[#22223a] flex items-center justify-center overflow-hidden">
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="flex gap-2">
                {post.tags?.map((tag: string, i: number) => (
                  <span key={i} className="bg-[#22223a] px-2 py-0.5 rounded text-[10px] uppercase font-bold text-white/60">{tag}</span>
                ))}
              </div>
            </div>
          </div>
        ...
        {post.video_url && (
          <div className="mb-12">
            <YouTubeEmbed url={post.video_url} className="shadow-2xl shadow-[#a020f0]/10" />
          </div>
        )}
        ...
        {post.youtube_embeds && post.youtube_embeds.length > 0 && (
          <section className="mb-16 pt-12 border-t border-[#22223a]">
            <h2 className="text-2xl font-display mb-8">Vidéos liées</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {post.youtube_embeds.map((url: string, i: number) => (
                <YouTubeEmbed key={i} url={url} className="border border-[#22223a]" />
              ))}
            </div>
          </section>
        )}
        ...
        {/* Related Formation */}
        {relatedFormation && (
          <section className="mb-16 p-8 bg-gradient-to-br from-[#12122a] to-[#0a0a1a] border border-[#a020f0]/30 rounded-[2rem] relative overflow-hidden group">
            ...
            <div className="relative z-10 flex flex-col md:flex-row items-center gap-8">
              ...
              <div className="flex-1 text-center md:text-left">
                ...
                <div className="flex items-center justify-center md:justify-start gap-6">
                  <span className="text-2xl font-display text-[#a020f0]">{relatedFormation.price_cents / 100}€</span>
                  <Link 
                    to={`/course/${relatedFormation.id}`}
                    className="bg-[#a020f0] hover:bg-[#b030ff] text-white px-6 py-2 rounded-full text-sm font-bold transition-all flex items-center gap-2"
                  >
        ...
        {recommendedPosts.length > 0 && (
          <section className="pt-12 border-t border-[#22223a]">
