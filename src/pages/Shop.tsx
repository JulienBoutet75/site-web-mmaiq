...
export function Shop() {
  ...
  return (
    <div className="bg-[#0a0a1a] text-white pt-32 pb-24 selection:bg-[#a020f0] selection:text-white min-h-screen">
      {/* Hero */}
      <section className="px-6 max-w-7xl mx-auto text-center mb-24 relative">
        ...
        >
          ...
          <h1 className="font-display text-5xl md:text-6xl lg:text-7xl mb-8 max-w-4xl mx-auto leading-[1.1] tracking-tighter drop-shadow-2xl">
            Équipement{" "}
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#a020f0] to-[#ff00ff] font-days-one tracking-normal">
              MMA IQ
            </span>
          </h1>
          <p className="font-body text-lg md:text-xl text-[#a0a0b8] max-w-3xl mx-auto mb-12 leading-relaxed">
            ...
            <button
              ...
              className={`px-6 py-2 rounded-full text-sm font-bold transition-all border ${
                category === cat 
                  ...
                  : "bg-white/5 border-white/10 text-[#a0a0b8] hover:border-white/20 hover:bg-white/10"
              }`}
            >
              {cat}
            </button>
      ...
      <section className="px-6 max-w-7xl mx-auto">
        {loading ? (
          ...
          <div className="text-center py-20 bg-white/5 rounded-[2rem] border border-white/10">
            <p className="text-[#a0a0b8] text-lg">Aucun produit trouvé dans cette catégorie.</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {filteredProducts.map((product, i) => (
              <motion.div
                ...
                className="h-full"
              >
                <div className="h-full flex flex-col group bg-[#12122a] border border-[#22223a] hover:border-[#a020f0]/30 transition-all duration-500 hover:-translate-y-2 hover:shadow-[0_20px_40px_-15px_rgba(160,32,240,0.2)] rounded-[2rem] overflow-hidden backdrop-blur-sm">
                  <div className="aspect-square bg-[#0a0a1a] relative overflow-hidden flex items-center justify-center">
                    ...
                    <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity duration-500 flex items-center justify-center z-20 backdrop-blur-sm">
                      <Link to={`/product/${product.slug}`}>
                        <Button variant="primary" className="translate-y-4 group-hover:translate-y-0 transition-all duration-500 shadow-[0_0_30px_rgba(160,32,240,0.4)] rounded-full px-6 py-3 flex items-center gap-2">
                  ...
                  <div className="p-8 flex flex-col flex-grow relative">
                    <div className="flex justify-between items-center mb-6">
                      <span className="text-[10px] font-bold tracking-widest uppercase text-[#a020f0] bg-[#a020f0]/10 px-3 py-1.5 rounded-full border border-[#a020f0]/20">
                      ...
                      <div className="flex gap-1">
                        {product.sizes?.slice(0, 3).map((s: string) => (
                          <span key={s} className="text-[10px] font-medium text-[#a0a0b8] border border-white/10 px-2 py-1 rounded bg-white/5">
                            {s}
                          </span>
                        ))}
                        {product.sizes?.length > 3 && <span className="text-[10px] text-[#6a6a82]">...</span>}
                      </div>
                    </div>
                    
                    <h3 className="font-display text-2xl mb-4 text-white group-hover:text-[#a020f0] transition-colors">
                      {product.name}
                    </h3>
                    
                    <p className="font-body text-[#a0a0b8] text-sm mb-8 flex-grow leading-relaxed line-clamp-2">
                      {product.description}
                    </p>
                    ...
                    <div className="flex justify-between items-center mt-auto pt-6 border-t border-white/5">
                      ...
                      <Button 
                        ...
                        variant="outline" 
                        className="px-6 py-3 rounded-full text-sm hover:bg-[#a020f0] hover:text-white hover:border-[#a020f0] transition-colors flex items-center gap-2 border-white/10 disabled:opacity-50"
                      >
